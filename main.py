...
outDirPath = f"{root_path}/images_out_2"
                    ...
                    if (
                        ...
                        for k, image in enumerate(sample["pred_xstart"]):
                            ...
                            if j % args.display_rate == 0 or cur_t == -1:
                                #image.save(f"progresses/{sys.argv[1]}/{str(j).zfill(4)}.png")
...
my_params = [
    # ["A photograph of elephant dung with a man next to it, smiling"],
    # ["A man smiling next to a photograph of elephant dung"],
    # ["Elephant dung photographed next to a smiling man"],
    # ["Photographed: Man smiles next to elephant dung"],
]

my_params = [
    (
        [
            f"World Trade Center on 9/11 with an orange sunset:10",
            f"matte painting by Simon Stalenhag:{x}",
            f"#vfxfriday, cryengine, trending on Artstation:{y}",
        ],
        (x, y),
    )
    for x, y in list(p([20], [16]))
]

my_params = [
    [
        "A vibrant illustration of a cyborg ninja, with bloody chainsaws for arms, matte painting by Simon Stalenhag"
    ]
]

# my_params = [
# 	([f"World Trade Center on 9/11 with an orange sunset:10", f"matte painting by Simon Stalenhag:{20}", f"#vfxfriday, cryengine, trending on Artstation:{10}"], z)
# 	for z in [1000, 2000, 5000, 10000, 20000, 50000]
# ]


my_params = [
    [
        "A vibrant illustration of an anime ninja character concept art, standing arms by side, matte painting by Simon Stalenhag, trending on Artstation"
    ]
]


def my_param_override(fname):
    pass


my_params = [[x for x in sys.argv[7:]]]
# my_params = [[sys.argv[4]]]

for i, tp in enumerate(my_params):

    mnb = int(sys.argv[2])

    batch_name = f"{sys.argv[1]}"  # @param{type: 'string'}
    steps = int(
        sys.argv[4]
    )  # @param [25,50,100,150,250,500,1000]{type: 'raw', allow-input: true}
    width_height = [1280, 768]  # @param{type: 'raw'}
    width_height = [448, 704]  # @param{type: 'raw'}
    ...
    if sys.argv[3] != "none":
        init_image = f"init_images/{sys.argv[3]}.png"
        if sys.argv[5] != "none":
                skip_steps = int(sys.argv[5])
        else:
                skip_steps = steps // 2
    else:
        skip_steps = 10
    ...
    display_rate = 100  # @param{type: 'number'}
    display_rate = 100  # @param{type: 'number'}
    # n_batches =  5 #@param{type: 'number'}
    n_batches = mnb  # @param{type: 'number'}
    ...
    timestep_respacing = f"ddim{steps}"
    seed = seed if sys.argv[6] == "none" else int(sys.argv[6])
    diffusion_steps = (1000 // steps) * steps if steps < 1000 else steps
    model_config.update(
        {
            "timestep_respacing": timestep_respacing,
